...
namespace Engine
{
	...
	void EditorCamera::OnUpdate(Timestep ts)
	{
		...
		if (Input::IsKeyPressed(Engine::Key::LeftAlt))
		{
			const glm::vec2& mouse{ Input::GetMouseX(), Input::GetMouseY()};
			glm::vec2 delta = (mouse - m_InitialMousePosition) * 0.003f;
			m_InitialMousePosition = mouse;

			if (Input::IsMouseButtonPressed(Engine::Mouse::ButtonMiddle))
				MousePan(delta);

			else if (Input::IsMouseButtonPressed( Engine::Mouse::ButtonLeft))
				MouseRotate(delta);

			else if (Input::IsMouseButtonPressed(Engine::Mouse::ButtonRight))
				MouseZoom(delta.y);
		}
